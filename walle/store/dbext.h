...
#include <walle/store/protodb.h>
#include <walle/store/stashdb.h>
#include <walle/store/cachedb.h>
#include <walle/store/hashdb.h>
#include <walle/store/dirdb.h>
#include <walle/store/polydb.h>
...
class MapReduce {
  ...
  bool execute(BasicDB* db, const std::string& tmppath = "", uint32_t opts = 0) {
    ...
    if (tmppath.empty()) {
      ...
      if (!File::status(tmppath, &sbuf) || !sbuf.isdir) {
        db->set_error(CODELINE, BasicDB::Error::NOREPOS, "no such directory");
        delete[] tmpdbs_;
        return false;
      }
      ...
      for (size_t i = 0; i < dbnum_; i++) {
        ...
        if (!tdb->open(childpath, TreeDB::OWRITER | TreeDB::OCREATE | TreeDB::OTRUNCATE)) {
          ...
          db->set_error(CODELINE, e.code(), e.message());
          err = true;
        }
        tmpdbs_[i] = tdb;
      }
    ...
    if (opts & XNOLOCK) {
      ...
      if (mapvisitor.error()) {
        db_->set_error(CODELINE, BasicDB::Error::LOGIC, "mapper failed");
        err = true;
      }
 ...
 protected:
  ...
  bool emit(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz) {
    assert(kbuf && ksiz <= MEMMAXSIZ && vbuf && vsiz <= MEMMAXSIZ);
    bool err = false;
    ...
    void run() {
      ...
      while ((kbuf = sorter.get(&ksiz, &vbuf, &vsiz)) != NULL) {
        if (!tmpdb->append(kbuf, ksiz, vbuf, vsiz)) {
          ...
          mr_->db_->set_error(CODELINE, e.code(), e.message());
          err_ = true;
        }
        sorter.step();
    ...
    while (!err && !lines.empty()) {
      ...
      if (lkbuf && (lksiz != line.ksiz || std::memcmp(lkbuf, line.kbuf, lksiz))) {
        if (!call_reducer(lkbuf, lksiz, values)) {
          db_->set_error(CODELINE, BasicDB::Error::LOGIC, "reducer failed");
          err = true;
        }
        values.clear();
      }
...
class IndexDB {
 ...
 public:
  ...
  void set_error(BasicDB::Error::Code code, const char* message) {
    assert(message);
    db_.set_error(CODELINE, code, message);
  }
  /**
  ...
  bool open(const std::string& path = ":",
    ...
    if (omode_ != 0) {
      set_error(CODELINE, BasicDB::Error::INVALID, "already opened");
      return false;
    }
    ...
    if ((mode & BasicDB::OWRITER) && dbnum > 0) {
      tmpdbs_ = new BasicDB*[dbnum_];
      if (tmppath_.empty()) {
        report(CODELINE, "started to open temporary databases on memory");
        double stime = time();
        for (size_t i = 0; i < dbnum_; i++) {
          GrassDB* gdb = new GrassDB;
        ...
        double etime = time();
        report(CODELINE, "opening temporary databases finished: time=%.6f", etime - stime);
      } else {
        File::Status sbuf;
        if (!File::status(tmppath_, &sbuf) || !sbuf.isdir) {
          set_error(CODELINE, BasicDB::Error::NOREPOS, "no such directory");
          delete[] tmpdbs_;
          db_.close();
          return false;
        }
        report(CODELINE, "started to open temporary databases under %s", tmppath.c_str());
        double stime = time();
        uint32_t pid = getpid() & UINT16MAX;
        uint32_t tid = Thread::hash() & UINT16MAX;
        uint32_t ts = time() * 1000;
        ...
        for (size_t i = 0; i < dbnum_; i++) {
          ...
          if (!tdb->open(childpath, TreeDB::OWRITER | TreeDB::OCREATE | TreeDB::OTRUNCATE)) {
            const BasicDB::Error& e = tdb->error();
            set_error(CODELINE, e.code(), e.message());
            err = true;
          }
          tmpdbs_[i] = tdb;
        }
  ...
  bool close() {
    ...
    ScopedRWLock lock(&mlock_, true);
    if (omode_ == 0) {
      set_error(CODELINE, BasicDB::Error::INVALID, "not opened");
      return false;
    }
    bool err = false;
    if (cache_) {
      ...
      if (tmpdbs_) {
        ...
        report(CODELINE, "closing the temporary databases");
        double stime = time();
        for (size_t i = 0; i < dbnum_; i++) {
        ...
        double etime = time();
        report(CODELINE, "closing the temporary databases finished: %.6f", etime - stime);
        delete[] tmpdbs_;
      }
    }
  ...
  bool set(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz) {
    ...
    if (!cache_) {
      set_error(CODELINE, BasicDB::Error::INVALID, "permission denied");
      return false;
    }
    bool err = false;
  ...
  bool add(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz) {
    ...
    if (check_impl(kbuf, ksiz)) {
      set_error(CODELINE, BasicDB::Error::DUPREC, "record duplication");
      return false;
    }
    bool err = false;
    cache_->set(kbuf, ksiz, vbuf, vsiz);
  ...
  bool replace(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz) {
    ...
    if (!check_impl(kbuf, ksiz)) {
      set_error(CODELINE, BasicDB::Error::NOREC, "no record");
      return false;
    }
    bool err = false;
    if (!clean_dbs(kbuf, ksiz)) err = true;
 ...
 private:
  ...
  bool flush_cache() {
    ...
    bool err = false;
    double stime = time();
    report(CODELINE, "flushing the cache");
    if (tmpdbs_) {
      BasicDB* tmpdb = tmpdbs_[dbclock_];
    ...
    csiz_ = 0;
    double etime = time();
    report(CODELINE, "flushing the cache finished: time=%.6f", etime - stime);
    return !err;
  }
  /**
  ...
  bool merge_tmpdbs() {
    assert(true);
    bool err = false;
    report(CODELINE, "merging the temporary databases");
    double stime = time();
    ...
    double etime = time();
    report(CODELINE, "merging the temporary databases finished: %.6f", etime - stime);
    return !err;
  }
  /**
