#ifndef WALLE_STORE_POLYDB_H                      // duplication check
#define WALLE_STORE_POLYDB_H
...
class PolyDB : public BasicDB {
 ...
 public:
  ...
  bool open(const std::string& path = ":", uint32_t mode = OWRITER | OCREATE) {
    ...
    switch (type) {
      default: {
        set_error(CODELINE, Error::INVALID, "unknown database type");
        return false;
      }
      case TYPEPHASH: {
    ...
    if (!db->open(fpath, mode)) {
      ...
      set_error(CODELINE, error.code(), error.message());
      delete db;
      return false;
    }
    if (arccomp) {
  ...
  int64_t match_regex(const std::string& regex, std::vector<std::string>* strvec,
    ...
    if (!reg.compile(regex, Regex::MATCHONLY)) {
      set_error(CODELINE, Error::LOGIC, "compilation failed");
      return -1;
    }
    bool err = false;
    int64_t allcnt = count();
  ...
  bool merge(BasicDB** srcary, size_t srcnum, MergeMode mode = MSET,
             ProgressChecker* checker = NULL) {
    assert(srcary && srcnum <= MEMMAXSIZ);
    if (type_ == TYPEVOID) {
      set_error(CODELINE, Error::INVALID, "not opened");
      return false;
    }
