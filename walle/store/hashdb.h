#ifndef WALLE_STORE_HASHDB_H                      // duplication check
#define WALLE_STORE_HASHDB_H
...
#define HDBMAGICDATA  "WT\n"           ///< magic data of the file
#define HDBCHKSUMSEED  "__wallestore__"  ///< seed of the module checksum
#define HDBTMPPATHEXT  "tmpwth"        ///< extension of the temporary file
...
class HashDB : public BasicDB {
 ...
 public:
  ...
  class Cursor : public BasicDB::Cursor {
   ...
   public:
    ...
    bool accept(Visitor* visitor, bool writable = true, bool step = false) {
      ...
      if (db_->comp_) {
        ...
        if (!zbuf) {
          db_->set_error(CODELINE, Error::SYSTEM, "data decompression failed");
          delete[] rec.bbuf;
          return false;
        }
        vbuf = zbuf;
      ...
      if (vbuf == Visitor::REMOVE) {
        ...
        if (db_->comp_) {
          ...
          if (!zbuf) {
            db_->set_error(CODELINE, Error::SYSTEM, "data compression failed");
            delete[] rec.bbuf;
            return false;
          }
          vbuf = zbuf;
    ...
    bool jump(const char* kbuf, size_t ksiz) {
      ...
      while (off > 0) {
        ...
        if (rec.psiz == UINT16MAX) {
          db_->set_error(CODELINE, Error::BROKEN, "free block in the chain");
          db_->report(CODELINE, Logger::WARN, "psiz=%lld off=%lld fsiz=%lld",
   ...
   private:
    ...
    bool step_impl(Record* rec, char* rbuf, int64_t skip) {
      assert(rec && rbuf && skip >= 0);
      if (off_ >= end_) {
        db_->set_error(CODELINE, Error::BROKEN, "cursor after the end");
        db_->report(CODELINE, Logger::WARN, "psiz=%lld off=%lld fsiz=%lld",
                    (long long)db_->psiz_, (long long)rec->off, (long long)db_->file_.size());
        return false;
      }
  ...
  bool open(const std::string& path, uint32_t mode = OWRITER | OCREATE) {
    ...
    if (file_.recovered()) report(CODELINE, Logger::WARN, "recovered by the WAL file");
    ...
    if (chksum != chksum_) {
      set_error(CODELINE, Error::INVALID, "invalid module checksum");
      report(CODELINE, Logger::WARN, "saved=%02X calculated=%02X",
    ...
    if (type_ == 0 || apow_ > MAXAPOW || fpow_ > MAXFPOW ||
      ...
      set_error(CODELINE, Error::BROKEN, "invalid meta data");
      report(CODELINE, Logger::WARN, "type=0x%02X apow=%d fpow=%d bnum=%lld count=%lld"
    ...
    if (file_.size() < lsiz_) {
      set_error(CODELINE, Error::BROKEN, "inconsistent file size");
      report(CODELINE, Logger::WARN, "lsiz=%lld fsiz=%lld",
  ...
  bool begin_transaction_try(bool hard = false) {
    ...
    if (tran_) {
      set_error(CODELINE, Error::LOGIC, "competition avoided");
      mlock_.unlock();
      return false;
    }
    trhard_ = hard;
  ...
  bool end_transaction(bool commit = true) {
    ...
    if (!tran_) {
      set_error(CODELINE, Error::INVALID, "not in transaction");
      return false;
    }
    bool err = false;
    if (commit) {
 ...
 private:
  ...
  class Repeater : public Visitor {
   ...
   private:
    ...
    const char* visit_full(const char* kbuf, size_t ksiz,
      ...
      assert(kbuf && ksiz <= MEMMAXSIZ && vbuf && vsiz <= MEMMAXSIZ && sp);
      *sp = vsiz_;
      return vbuf_;
    }
    const char* vbuf_;
  ...
  bool accept_impl(const char* kbuf, size_t ksiz, Visitor* visitor,
    ...
    assert(kbuf && ksiz <= MEMMAXSIZ && visitor && bidx >= 0);
    ...
    while (off > 0) {
      ...
      if (rec.psiz == UINT16MAX) {
        set_error(CODELINE, Error::BROKEN, "free block in the chain");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld fsiz=%lld",
      ...
      if (pivot > tpivot) {
        ...
        if (kcmp > 0) {
          ...
          if (comp_) {
            ...
            if (!zbuf) {
              set_error(CODELINE, Error::SYSTEM, "data decompression failed");
              delete[] rec.bbuf;
              return false;
            }
            vbuf = zbuf;
          ...
          if (vbuf == Visitor::REMOVE) {
            ...
            if (comp_ && !isiter) {
              ...
              if (!zbuf) {
                set_error(CODELINE, Error::SYSTEM, "data compression failed");
                delete[] rec.bbuf;
                return false;
              }
              vbuf = zbuf;
          ...
          while (off > 0 && off < end) {
            ...
            if (rec.psiz == UINT16MAX) {
              ...
              if (comp) {
                ...
                if (!zbuf) {
                  db->set_error(CODELINE, Error::SYSTEM, "data decompression failed");
                  delete[] rec.bbuf;
                  error_ = db->error();
                  break;
                }
  ...
  bool load_meta() {
    ...
    if (file_.size() < (int64_t)sizeof(head)) {
      set_error(CODELINE, Error::INVALID, "missing magic data of the file");
      return false;
    }
    ...
    if (std::memcmp(head, HDBMAGICDATA, sizeof(HDBMAGICDATA))) {
      set_error(CODELINE, Error::INVALID, "invalid magic data of the file");
      return false;
    }
  ...
  bool reorganize_file(const std::string& path) {
    ...
    if (db.open(npath, OWRITER | OCREATE | OTRUNCATE)) {
      report(CODELINE, Logger::WARN, "reorganizing the database");
      lsiz_ = file_.size();
      psiz_ = lsiz_;
      if (copy_records(&db)) {
        if (db.close()) {
          if (!File::rename(npath, path)) {
            set_error(CODELINE, Error::SYSTEM, "renaming the destination failed");
            err = true;
          }
        } else {
          set_error(CODELINE, db.error().code(), "closing the destination failed");
          err = true;
        }
      } else {
        set_error(CODELINE, db.error().code(), "record copying failed");
        err = true;
      }
      File::remove(npath);
    } else {
      set_error(CODELINE, db.error().code(), "opening the destination failed");
      err = true;
    }
    return !err;
  }
  ...
  bool read_record(Record* rec, char* rbuf) {
    assert(rec && rbuf);
    if (rec->off < roff_) {
      set_error(CODELINE, Error::BROKEN, "invalid record offset");
      report(CODELINE, Logger::WARN, "psiz=%lld off=%lld fsiz=%lld",
             (long long)psiz_, (long long)rec->off, (long long)file_.size());
      return false;
    }
    ...
    if (rsiz > RECBUFSIZ) {
      ...
      if (rsiz < rhsiz_) {
        set_error(CODELINE, Error::BROKEN, "too short record region");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld",
    ...
    if (*(uint8_t*)rp == RECMAGIC) {
      ...
      if (*(uint8_t*)(rp++) != FBMAGIC || *(uint8_t*)(rp++) != FBMAGIC) {
        set_error(CODELINE, Error::BROKEN, "invalid magic data of a free block");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld",
               (long long)psiz_, (long long)rec->off, (long long)rsiz, (long long)file_.size());
        report_binary(CODELINE, Logger::WARN, "rbuf", rbuf, rsiz);
        return false;
      }
      ...
      if (rec->rsiz < rhsiz_) {
        set_error(CODELINE, Error::BROKEN, "invalid size of a free block");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld",
               (long long)psiz_, (long long)rec->off, (long long)rsiz, (long long)file_.size());
        report_binary(CODELINE, Logger::WARN, "rbuf", rbuf, rsiz);
        return false;
      ...
      set_error(CODELINE, Error::BROKEN, "nullified region");
      report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld",
             (long long)psiz_, (long long)rec->off, (long long)rsiz, (long long)file_.size());
      report_binary(CODELINE, Logger::WARN, "rbuf", rbuf, rsiz);
      return false;
    ...
    if (step < 1) {
      set_error(CODELINE, Error::BROKEN, "invalid key length");
      report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld snum=%04X",
    ...
    if (step < 1) {
      set_error(CODELINE, Error::BROKEN, "invalid value length");
      report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld snum=%04X",
             (long long)psiz_, (long long)rec->off, (long long)rsiz,
             (long long)file_.size(), snum);
      report_binary(CODELINE, Logger::WARN, "rbuf", rbuf, rsiz);
    ...
    if (rsiz >= rec->ksiz) {
      ...
      if (rsiz >= rec->vsiz) {
        ...
        if (rec->psiz > 0) {
          ...
          if (rsiz > 0 && *(uint8_t*)rp != PADMAGIC) {
            set_error(CODELINE, Error::BROKEN, "invalid magic data of a record");
            report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld"
      ...
      if (rec->off + (int64_t)rec->rsiz > psiz_) {
        set_error(CODELINE, Error::BROKEN, "invalid length of a record");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld"
  ...
  bool read_record_body(Record* rec) {
    ...
    if (rec->psiz > 0 && ((uint8_t*)bbuf)[bsiz-1] != PADMAGIC) {
      set_error(CODELINE, Error::BROKEN, "invalid magic data of a record");
      report_binary(CODELINE, Logger::WARN, "bbuf", bbuf, bsiz);
      delete[] bbuf;
      return false;
    }
    rec->bbuf = bbuf;
  ...
  bool shift_record(Record* orec, int64_t dest) {
    ...
    set_error(CODELINE, Error::BROKEN, "no record to shift");
    report(CODELINE, Logger::WARN, "psiz=%lld fsiz=%lld",
  ...
  bool load_free_blocks() {
    ...
    while (num < fbpnum_ && size > 1 && *rp != '\0') {
      ...
      if (step < 1 || off < 1) {
        set_error(CODELINE, Error::BROKEN, "invalid free block offset");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld fsiz=%lld",
      ...
      if (step < 1 || rsiz < 1) {
        set_error(CODELINE, Error::BROKEN, "invalid free block size");
        report(CODELINE, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld",
  ...
  bool cut_chain(Record* rec, char* rbuf, int64_t bidx, int64_t entoff) {
    assert(rec && rbuf && bidx >= 0 && entoff >= 0);
    int64_t child;
    if (rec->left > 0 && rec->right < 1) {
      ...
      if (prec.psiz == UINT16MAX) {
        ...
        report_binary(CODELINE, Logger::WARN, "rbuf", rbuf, rhsiz_);
        return false;
      }
      delete[] prec.bbuf;
      if (prec.right > 0) {
