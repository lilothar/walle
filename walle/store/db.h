#ifndef WALLE_STORE_DB_H                          // duplication check
#define WALLE_STORE_DB_H
...
class BasicDB : public DB {
 ...
 public:
  ...
  bool add(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz) {
    ...
    if (!accept(kbuf, ksiz, &visitor, true)) return false;
    if (!visitor.ok()) {
      set_error(CODELINE, Error::DUPREC, "record duplication");
      return false;
    }
    return true;
  }
  ...
  bool replace(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz) {
    ...
    if (!accept(kbuf, ksiz, &visitor, true)) return false;
    if (!visitor.ok()) {
      set_error(CODELINE, Error::NOREC, "no record");
      return false;
    }
    return true;
  }
  ...
  int64_t increment(const char* kbuf, size_t ksiz, int64_t num, int64_t orig = 0) {
    ...
    num = visitor.num();
    if (num == INT64MIN) {
      set_error(CODELINE, Error::LOGIC, "logical inconsistency");
      return num;
    }
    return num;
  }
  ...
  bool cas(const char* kbuf, size_t ksiz,
    ...
    if (!accept(kbuf, ksiz, &visitor, true)) return false;
    if (!visitor.ok()) {
      set_error(CODELINE, Error::LOGIC, "status conflict");
      return false;
    }
    return true;
  }
  ...
  int32_t get(const char* kbuf, size_t ksiz, char* vbuf, size_t max) {
    assert(kbuf && ksiz <= MEMMAXSIZ && vbuf);
    class VisitorImpl : public Visitor {
     public:
  ...
  bool dump_snapshot(std::ostream* dest, ProgressChecker* checker = NULL) {
    assert(dest);
    if (dest->fail()) {
      set_error(CODELINE, Error::INVALID, "invalid stream");
      return false;
    }
    class VisitorImpl : public Visitor {
     public:
    ...
    if (iterate(&visitor, false, checker)) {
      ...
      dest->write((char*)&c, 1);
      if (dest->fail()) {
        set_error(CODELINE, Error::SYSTEM, "stream output error");
        err = true;
      }
    } else {
      err = true;
  ...
  bool dump_snapshot(const std::string& dest, ProgressChecker* checker = NULL) {
    ...
    if (!ofs) {
      set_error(CODELINE, Error::NOREPOS, "open failed");
      return false;
    }
    bool err = false;
    ...
    ofs.close();
    if (!ofs) {
      set_error(CODELINE, Error::SYSTEM, "close failed");
      err = true;
    }
    return !err;
  }
  ...
  bool load_snapshot(std::istream* src, ProgressChecker* checker = NULL) {
    ...
    if (src->fail()) {
      set_error(CODELINE, Error::SYSTEM, "stream input error");
      return false;
    }
    if (std::memcmp(buf, KCDBSSMAGICDATA, sizeof(KCDBSSMAGICDATA))) {
      set_error(CODELINE, Error::INVALID, "invalid magic data of input stream");
      return false;
    }
    bool err = false;
