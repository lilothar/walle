...
#ifndef WALLE_STORE_PLANTDB_H                     // duplication check
#define WALLE_STORE_PLANTDB_H
...
#define PDBMETAKEY  "@"                ///< key of the record for meta data
#define PDBTMPPATHEXT  "tmpkct"        ///< extension of the temporary file
#define PDRECBUFSIZ  128               ///< size of the record buffer
...
class PlantDB : public BasicDB {
 ...
 public:
   ...
   private:
    ...
    bool set_position(int64_t id) {
      ...
      while (id > 0) {
        ...
        if (!node) {
          db_->set_error(CODELINE, Error::BROKEN, "missing leaf node");
          db_->db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)id);
          return false;
        }
          ...
          if (link || flush || async) {
            ...
            if (link) {
              ...
              if (node) {
                ...
                db_->set_error(CODELINE, Error::BROKEN, "search failed");
                err = true;
              }
            } else if (flush) {
    ...
    bool accept_atom(Visitor* visitor, bool step, bool *retryp) {
      ...
      *retryp = false;
      ...
      if (node->recs.empty()) {
        ...
        node = db_->search_tree(link, true, hist, &hnum);
        if (node->id != lid_) {
          db_->set_error(CODELINE, Error::BROKEN, "invalid tree");
          if (lbuf != lstack) delete[] lbuf;
          return false;
        }
      }
      ...
      if (rit != ritend) {
        ...
        if (back_) {
          if (set_position_back(node->prev)) {
            if (lid_ == lid) {
              db_->set_error(CODELINE, Error::BROKEN, "invalid leaf node");
              err = true;
            } else {
              *retryp = true;
            }
  ...
  bool accept(const char* kbuf, size_t ksiz, Visitor* visitor, bool writable = true) {
    ...
    if (!node) {
      set_error(CODELINE, Error::BROKEN, "search failed");
      if (lbuf != lstack) delete[] lbuf;
      mlock_.unlock();
      return false;
    }
  ...
  bool iterate(Visitor *visitor, bool writable = true, ProgressChecker* checker = NULL) {
    ...
    while (!err && id > 0) {
      ...
      if (!node) {
        set_error(CODELINE, Error::BROKEN, "missing leaf node");
        db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)id);
        return false;
      }
      id = node->next;
      const RecordArray& recs = node->recs;
  ...
  bool scan_parallel(Visitor *visitor, size_t thnum, ProgressChecker* checker = NULL) {
      ...
      const char* visit_full(const char* kbuf, size_t ksiz,
        ...
        while (vsiz > 1) {
          ...
          if (checker_ && !checker_->check("scan_parallel", "processing", -1, allcnt_)) {
            db_->set_error(CODELINE, Error::LOGIC, "checker failed");
            error_ = db_->error();
            ichecker_->stop();
            break;
          }
    ...
    if (ivisitor.error() != Error::SUCCESS) {
      ...
      db_.set_error(CODELINE, e.code(), e.message());
      err = true;
    }
  ...
  bool open(const std::string& path, uint32_t mode = OWRITER | OCREATE) {
    ...
    if (db_.type() != DBTYPE) {
      set_error(CODELINE, Error::INVALID, "invalid database type");
      db_.close();
      return false;
    }
    if (db_.reorganized()) {
    ...
    if (psiz_ < 1 || root_ < 1 || first_ < 1 || last_ < 1 ||
      ...
      db_.report(CODELINE, Logger::WARN, "psiz=%lld root=%lld first=%lld last=%lld"
  ...
  bool close() {
    ...
    report(CODELINE, Logger::DEBUG, "closing the database (path=%s)", path.c_str());
    bool err = false;
    disable_cursors();
    int64_t lsiz = calc_leaf_cache_size();
    int64_t isiz = calc_inner_cache_size();
    if (cusage_ != lsiz + isiz) {
      set_error(CODELINE, Error::BROKEN, "invalid cache usage");
      db_.report(CODELINE, Logger::WARN, "cusage=%lld lsiz=%lld isiz=%lld",
    ...
    if (cusage_ != 0 || lsiz != 0 || isiz != 0 || lcnt != 0 || icnt != 0) {
      set_error(CODELINE, Error::BROKEN, "remaining cache");
      db_.report(CODELINE, Logger::WARN, "cusage=%lld lsiz=%lld isiz=%lld"
 ...
 private:
  ...
  LeafNode* divide_leaf_node(LeafNode* node) {
    ...
    if (newnode->next > 0) {
      ...
      if (!nextnode) {
        set_error(CODELINE, Error::BROKEN, "missing leaf node");
        db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)newnode->next);
        return NULL;
      }
  ...
  LeafNode* search_tree(Link* link, bool prom, int64_t* hist, int32_t* hnp) {
    assert(link && hist && hnp);
    int64_t id = root_;
    int32_t hnum = 0;
    while (id > INIDBASE) {
      InnerNode* node = load_inner_node(id);
      if (!node) {
        set_error(CODELINE, Error::BROKEN, "missing inner node");
        db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)id);
        return NULL;
      }
      hist[hnum++] = id;
  ...
  bool reorganize_tree(LeafNode* node, int64_t* hist, int32_t hnum) {
    ...
    if (node->size > psiz_ && node->recs.size() > 1) {
      ...
      while (true) {
        ...
        if (!inode) {
          set_error(CODELINE, Error::BROKEN, "missing inner node");
          db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)parent);
          delete[] kbuf;
          return false;
        }
        add_link_inner_node(inode, child, kbuf, ksiz);
      ...
      if (!inode) {
        set_error(CODELINE, Error::BROKEN, "missing inner node");
        db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)hist[hnum]);
        return false;
      }
      if (sub_link_tree(inode, node->id, hist, hnum)) {
        if (node->prev > 0) {
          ...
          if (!tnode) {
            set_error(CODELINE, Error::BROKEN, "missing node");
            db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)node->prev);
            return false;
          }
        ...
        if (node->next > 0) {
          ...
          if (!tnode) {
            set_error(CODELINE, Error::BROKEN, "missing node");
            db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)node->next);
            return false;
          }
  ...
  bool sub_link_tree(InnerNode* node, int64_t child, int64_t* hist, int32_t hnum) {
    ...
    if (node->heir == child) {
      ...
      while (child > INIDBASE) {
        ...
        if (!node) {
          set_error(CODELINE, Error::BROKEN, "missing inner node");
          db_.report(CODELINE, Logger::WARN, "id=%lld", (long long)child);
          return false;
        }
        if (node->dead) {
          child = node->heir;
    ...
    set_error(CODELINE, Error::BROKEN, "invalid tree");
    return false;
  }
  /**
  ...
  bool load_meta() {
    ...
    if (hsiz != sizeof(head)) {
      set_error(CODELINE, Error::BROKEN, "invalid meta data record");
      db_.report(CODELINE, Logger::WARN, "hsiz=%d", hsiz);
      return false;
    }
    const char* rp = head;
    if (*(uint8_t*)rp == 0x10) {
      ...
      if (!reccomp_.comp) {
        set_error(CODELINE, Error::INVALID, "the custom comparator is not given");
        return false;
      }
      linkcomp_.comp = reccomp_.comp;
    } else {
      set_error(CODELINE, Error::BROKEN, "comparator is invalid");
      return false;
    }
    rp = head + MOFFNUMS;
    uint64_t num;
  ...
  bool recalc_count() {
    ...
    db_.report(CODELINE, Logger::WARN, "recalculated the record count from %lld to %lld",
    ...
    while (nit != nitend) {
      if (ids.find(*nit) == ids.end()) {
        db_.report(CODELINE, Logger::WARN, "detected missing leaf: %lld", (long long)*nit);
        count = INT64MAX;
      }
      ++nit;
    }
    ...
    while (pit != pitend) {
      if (ids.find(*pit) == iitend) {
        db_.report(CODELINE, Logger::WARN, "detected missing leaf: %lld", (long long)*pit);
        count = INT64MAX;
      }
      ++pit;
    }
  ...
  bool reorganize_file(uint32_t mode) {
    ...
    if (!tdb.open(npath, OWRITER | OCREATE | OTRUNCATE)) {
      set_error(CODELINE, tdb.error().code(), "opening the destination failed");
      return false;
    }
    db_.report(CODELINE, Logger::WARN, "reorganizing the database");
    bool err = false;
    create_leaf_cache();
    create_inner_cache();
          ...
          if (node) {
            ...
            while (rit != ritend) {
              ...
              if (!tdb.set(dbuf, rec->ksiz, dbuf + rec->ksiz, rec->vsiz)) {
                set_error(CODELINE, tdb.error().code(),
    ...
    if (DBTYPE == TYPETREE) {
      ...
      if (File::rename(path, tpath)) {
        ...
        set_error(CODELINE, Error::SYSTEM, "renaming the source failed");
        err = true;
      }
      File::remove_recursively(tpath);
      ...
      if (!err && udb.open(npath, OREADER)) {
        ...
        if (!udb.close()) {
          set_error(CODELINE, udb.error().code(), "closing the destination failed");
          err = true;
        }
      } else {
        set_error(CODELINE, udb.error().code(), "opening the destination failed");
        err = true;
      }
      File::remove_recursively(npath);
    }
