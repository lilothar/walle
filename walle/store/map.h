#ifndef WALLE_STORE_MAP_H                   
#define WALLE_STORE_MAP_H
 ...
 private:
  ...
  struct Record {
    ...
    Record(char* child, const char* kbuf, uint64_t ksiz,
      ...
      assert(kbuf && ksiz <= MEMMAXSIZ && vbuf && vsiz <= MEMMAXSIZ && psiz <= MEMMAXSIZ);
    }
    /** constructor */
    ...
    void overwrite(char* rbuf, const char* vbuf, size_t vsiz, size_t psiz) {
      assert(rbuf && vbuf && vsiz <= MEMMAXSIZ && psiz <= MEMMAXSIZ);
    ...
    void concatenate(char* rbuf, const char* ovbuf, size_t ovsiz,
      ...
      assert(rbuf && ovbuf && ovsiz <= MEMMAXSIZ && nvbuf && nvsiz <= MEMMAXSIZ);
      char* wp = rbuf + sizeof(child_) + sizevarnum(ksiz_) + ksiz_;
