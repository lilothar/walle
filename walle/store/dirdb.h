#ifndef WALLE_STORE_DIRDB_H                       // duplication check
#define WALLE_STORE_DIRDB_H
...
#define DDBMAGICFILE  "__WALLEDIR__"      ///< magic file of the directory
#define DDBMETAFILE  "__meta__"        ///< meta data file of the directory
#define DDBOPAQUEFILE  "__opq__"       ///< opaque file of the directory
#define DDBATRANPREFIX  "_x"           ///< prefix of files for auto transaction
#define DDBCHKSUMSEED  "__wallestore__"  ///< seed of the module checksum
#define DDBMAGICEOF  "_EOF_"           ///< magic data for the end of file
#define DDBWALPATHEXT  "wal"           ///< extension of the WAL directory
#define DDBTMPPATHEXT  "tmp"           ///< extension of the temporary directory
...
class DirDB : public BasicDB {
  ...
  class Cursor : public BasicDB::Cursor {
   ...
   public:
    ...
    bool jump() {
      ...
      if (!dir_.open(db_->path_)) {
        db_->set_error(CODELINE, Error::SYSTEM, "opening a directory failed");
        return false;
      }
      alive_ = true;
      do {
   ...
   private:
    ...
    bool disable() {
      bool err = false;
      if (!dir_.close()) {
        db_->set_error(CODELINE, Error::SYSTEM, "closing a directory failed");
        err = true;
      }
      alive_ = false;
      return !err;
  ...
  bool open(const std::string& path, uint32_t mode = OWRITER | OCREATE) {
    ...
    if (writer_ && (mode & OTRUNCATE) && File::status(magicpath)) {
      ...
      if (!File::write_file(magicpath, buf.c_str(), buf.size())) {
        set_error(CODELINE, Error::SYSTEM, "writing a file failed");
        return false;
      }
      if (File::status(metapath) && !File::remove(metapath)) {
        set_error(CODELINE, Error::SYSTEM, "removing a file failed");
        return false;
      }
    ...
    if (File::status(cpath, &sbuf)) {
      if (!sbuf.isdir) {
        set_error(CODELINE, Error::NOPERM, "invalid path (not directory)");
        return false;
      }
      if (!File::status(magicpath)) {
        set_error(CODELINE, Error::BROKEN, "invalid magic data");
        return false;
      }
      if (!file_.open(magicpath, fmode)) {
        set_error(CODELINE, Error::SYSTEM, file_.error());
      ...
      if (!File::make_directory(cpath)) {
        set_error(CODELINE, Error::SYSTEM, "making a directory failed");
        return false;
      }
      if (!file_.open(magicpath, fmode)) {
        set_error(CODELINE, Error::SYSTEM, file_.error());
      ...
      set_error(CODELINE, Error::NOREPOS, "open failed (file not found)");
      return false;
    }
    if (hot) {
      count_ = 0;
      ...
      if (File::status(walpath, &sbuf)) {
        ...
        if (dir.open(walpath)) {
          ...
          File::remove_directory(walpath);
          recov_ = true;
          report(CODELINE, Logger::WARN, "recovered by the WAL directory");
        }
      }
      ...
      if (!load_magic()) {
        ...
        report(CODELINE, Logger::WARN, "re-calculated magic data");
      }
    }
      ...
      if (dir.open(path_)) {
        ...
        while (dir.read(&name)) {
          ...
          if (File::status(walpath)) {
            ...
            set_error(CODELINE, Error::SYSTEM, "renaming a file failed");
            err = true;
          }
        }
        if (!dir.close()) {
          set_error(CODELINE, Error::SYSTEM, "closing a directory failed");
          err = true;
        }
      } else {
        set_error(CODELINE, Error::SYSTEM, "opening a directory failed");
        err = true;
      }
    } else {
 ...
 private:
  ...
  bool calc_magic(const std::string& cpath) {
    ...
    while (dir.read(&name)) {
      ...
      if (File::status(rpath, &sbuf)) {
        ...
        set_error(CODELINE, Error::SYSTEM, "checking the status of a file failed");
        err = true;
      }
    }
    if (!dir.close()) {
  ...
  bool load_meta(const std::string& metapath) {
    ...
    if (!buf) {
      set_error(CODELINE, Error::SYSTEM, "reading a file failed");
      return false;
    }
    ...
    if (strsplit(str, '\n', &elems) < 7 || elems[6] != DDBMAGICEOF) {
      set_error(CODELINE, Error::BROKEN, "invalid meta data file");
      return false;
    }
  ...
  bool read_record(const std::string& rpath, Record* rec) {
    ...
    if (rsiz < 4 || *(const unsigned char*)rp != RECMAGIC) {
      ...
      report(CODELINE, Logger::WARN, "rpath=%s", rpath.c_str());
    ...
    if (rsiz < 1 + (int64_t)ksiz + (int64_t)vsiz ||
      ...
      set_error(CODELINE, Error::BROKEN, "too short record");
      report(CODELINE, Logger::WARN, "rpath=%s", rpath.c_str());
      delete[] rbuf;
      return false;
    }
  ...
  bool write_record(const std::string& rpath, const char* name, const char* kbuf, size_t ksiz,
    ...
    assert(kbuf && ksiz <= MEMMAXSIZ && vbuf && vsiz <= MEMMAXSIZ && wsp);
    bool err = false;
  ...
  bool accept_impl(const char* kbuf, size_t ksiz, Visitor* visitor, const char* name) {
    assert(kbuf && ksiz <= MEMMAXSIZ && visitor && name);
    bool err = false;
    const std::string& rpath = path_ + File::PATHCHR + name;
    Record rec;
    if (read_record(rpath, &rec)) {
      if (rec.ksiz == ksiz || !std::memcmp(rec.kbuf, kbuf, ksiz)) {
        ...
        set_error(CODELINE, Error::LOGIC, "collision of the hash values");
        err = true;
      }
      delete[] rec.rbuf;
    } else {
  ...
  bool accept_visit_full(const char* kbuf, size_t ksiz, const char* vbuf, size_t vsiz,
                         ...
                         const char* name) {
    assert(kbuf && ksiz <= MEMMAXSIZ && vbuf && vsiz <= MEMMAXSIZ && visitor);
    bool err = false;
    size_t rsiz;
  ...
  bool iterate_impl(Visitor* visitor, ProgressChecker* checker) {
    ...
    while (dir.read(&name)) {
      ...
      if (read_record(rpath, &rec)) {
        ...
        set_error(CODELINE, Error::BROKEN, "missing record");
        err = true;
      }
      curcnt++;
  ...
  bool commit_transaction() {
    ...
    if (!File::rename(walpath_, tmppath_)) {
      set_error(CODELINE, Error::SYSTEM, "renaming a directory failed");
      err = true;
    }
    if (!remove_files(tmppath_)) err = true;
    if (!File::remove_directory(tmppath_)) {
      set_error(CODELINE, Error::SYSTEM, "removing a directory failed");
      return false;
    }
    if (trhard_ && !File::synchronize_whole()) {
      set_error(CODELINE, Error::SYSTEM, "synchronizing the file system failed");
    ...
    if (dir.open(walpath_)) {
      ...
      while (dir.read(&name)) {
        ...
        if (File::status(srcpath, &sbuf)) {
          ...
          set_error(CODELINE, Error::SYSTEM, "checking a file failed");
          err = true;
        }
      }
      if (!dir.close()) {
