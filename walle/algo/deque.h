#ifndef WALLE_DEQUE_H__
#define WALLE_DEQUE_H__
...
protected:
    ...
    typedef default_alloc<value_type, Alloc> data_allocator;
    typedef default_alloc<pointer, Alloc> map_allocator;
public:
    // construct
...
deque<T, Alloc, BufSize>::insert_aux(iterator pos, const value_type& x) {
    ...
    if (index < (size() >> 1)) {
        ...
        ++front2;
        pos = start + index;    // ?????,??start??pish_front()????
        iterator pos1 = pos;
        ++pos1;
...
deque<T, Alloc, BufSize>::erase(iterator first, iterator last) {
    if (first == start && last == finish) {
        ...
        if (elems_before < (size() - n) / 2) {
            ...
            // destory(start.cur, new_start.cur);???????
